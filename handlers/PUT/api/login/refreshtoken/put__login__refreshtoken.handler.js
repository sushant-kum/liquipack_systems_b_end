...
exports.handler = function (req, res) {
    try {
        ...
        if (valid_arguments_flag) {
            check_app_permission(app_config.app_names.token, app_config.permissions.write, req.body.username, function (error, is_permitted = null) {
                if (error) {
                                ...
                                if (jwt_verify.username == req.body.username) {
                                    TokenController.findOne({
                                        ...
                                        if (err) {
                                            ...
                                            const new_token = jwt.sign({ username: req.body.username }, jwt_config.token_secret, { expiresIn: jwt_config.token_life });
