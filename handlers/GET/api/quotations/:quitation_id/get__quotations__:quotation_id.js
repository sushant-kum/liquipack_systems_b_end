...
let QuotationController = require(base_path + "/controllers/apps-quotation.controller.js");

exports.handler = function (req, res) {
  try {
    check_app_permission(app_config.app_names.apps_quotation, app_config.permissions.read, req.user.username, function (error, is_permitted = null) {
      if (error) {
        ...
        if (is_permitted) {
          ...
          if (valid_arguments_flag) {
            QuotationController.view(req.params.quotation_id, function (error, quotation = null) {
              if (error) {
                return res.status(500).json({
                  status: "error",
                  message: error,
