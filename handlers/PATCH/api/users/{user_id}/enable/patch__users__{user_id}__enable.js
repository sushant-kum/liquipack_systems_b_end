...
exports.handler = function(req, res) {
  try {
    check_app_permission(app_config.app_names.system_users, app_config.permissions.write, req.user.username, function(
      error,
      is_permitted = null
    ) {
      if (error) {
        ...
        if (is_permitted) {
          ...
          if (valid_arguments_flag) {
            UserController.make_active(req.params.user_id, function(error, user = null) {
              if (error) {
                return res.status(500).json({
                  status: 'error',
                  message: error,
