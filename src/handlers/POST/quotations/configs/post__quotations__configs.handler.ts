...
import { NativeScehma } from '@app/models/mongoose-db/apps-quotation-config.model';
        ...
        if (error) {
          ...
          if (is_permitted) {
            ...
            if (valid_arguments_flag) {
              const new_quotation_config: NativeScehma = {
              ...
              QuotationConfigController.create(
                new_quotation_config,
                (errorQuotationConfigControllercreate, quotation_config) => {
                  if (errorQuotationConfigControllercreate) {
                    return res.status(500).json({
                      status: 'error',
                      message: errorQuotationConfigControllercreate,
                      user_id: req.user._id,
                      token: req.user.token
                    });
                  } else {
                    return res.json({
                      status: 'success',
                      message: 'Quotation config created successfully',
