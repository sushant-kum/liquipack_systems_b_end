      ...
      if (error) {
        ...
        if (is_permitted) {
          ...
          if (valid_arguments_flag) {
            QuotationController.view(req.params.quotation_id, (errorQuotationControllerview, quotation) => {
              if (errorQuotationControllerview) {
                return res.status(500).json({
                  status: 'error',
                  message: errorQuotationControllerview,
                  user_id: req.user._id,
                  token: req.user.token
                });
              } else {
