        ...
        if (error) {
          ...
          if (is_permitted) {
            ...
            if (valid_arguments_flag) {
              QuotationConfigController.view(
                req.params.config_id,
                (errorQuotationConfigControllerview, quotation_config) => {
                  if (errorQuotationConfigControllerview) {
                    return res.status(500).json({
                      status: 'error',
                      message: errorQuotationConfigControllerview,
                      user_id: req.user._id,
                      token: req.user.token
                    });
                  } else {
