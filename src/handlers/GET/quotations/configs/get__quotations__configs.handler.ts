    ...
    checkAppPermission(
      ...
      (error, is_permitted) => {
        if (error) {
          ...
          if (is_permitted) {
            QuotationConfigController.index((errorQuotationConfigControllerindex, quotation_configs) => {
              if (errorQuotationConfigControllerindex) {
                return res.status(500).json({
                  status: 'error',
                  message: errorQuotationConfigControllerindex,
                  user_id: req.user._id,
                  token: req.user.token
                });
              } else {
