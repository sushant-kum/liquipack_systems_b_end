      ...
      if (error) {
        ...
        if (is_permitted) {
          ...
          if (valid_arguments_flag) {
            QuotationController.remove(req.params.quotation_id, errorQuotationControllerremove => {
              if (errorQuotationControllerremove) {
                return res.status(500).json({
                  status: 'error',
                  message: errorQuotationControllerremove,
                  user_id: req.user._id,
                  token: req.user.token
                });
              } else {
