        ...
        if (error) {
          ...
          if (is_permitted) {
            ...
            if (valid_arguments_flag) {
              QuotationConfigController.remove(req.params.config_id, errorQuotationConfigControllerremove => {
                if (errorQuotationConfigControllerremove) {
                  return res.status(500).json({
                    status: 'error',
                    message: errorQuotationConfigControllerremove,
                    user_id: req.user._id,
                    token: req.user.token
                  });
                } else {
                  return res.json({
                    status: 'success',
                    message: 'Quotation config deleted permanently',
                    data: req.params.config_id,
                    user_id: req.user._id,
                    token: req.user.token
                  });
                }
