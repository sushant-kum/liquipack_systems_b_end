            ...
            TokenController.findOne({
                ...
                if (token_record) {
                    if (token_record.uuid == jwt_payload.uuid) {
                        const uuid = uuidv4();
                        const new_token = jwt.sign({ username: user.username, uuid: uuid }, jwt_config.token_secret, { expiresIn: jwt_config.token_life });
                        TokenController.update(token_record._id, {
                            token: new_token,
                            uuid: uuid
